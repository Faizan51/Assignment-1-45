...
//Code Solution
let currentUsers = ["Farhad", "Azam", "Misbah", "Umair", "Abrar", "Ehtisham"];
let newUsers = ["Hammad", "Asghar", "Umair", "Misbah", "Hassan", "Ali"];
newUsers.forEach((newOneUser => {
    let ourCondition = currentUsers.some(currentNewOne => currentNewOne.toLowerCase() === newOneUser.toLowerCase());
    if (ourCondition) {
        console.log(`This ${newOneUser} is already avalaible in current user Please enter new user name  `);
    }
    else {
        console.log(`This ${newOneUser} is avalaible`);
    }
}));
export {};
